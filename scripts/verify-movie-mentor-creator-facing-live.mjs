...
function applyTurn(message,journey,intelligence,synthesis){const fakeResult={response:{text:synthesis?.text||"",structured:{movieJourneyIntelligence:intelligence}},movieJourneyIntelligence:intelligence,mentorSynthesis:synthesis?{status:"completed"}:{status:"blocked-by-clarification"},content:synthesis?.text||"",prompt:synthesis?.text||"",preview:synthesis?.text||""};const applied=bridge.applyGenerationResult(journey,fakeResult,{originalIdea:message,source:"live-creator-facing-verifier"});const presentation=resolveMovieMentorCreatorFacingMessage({result:fakeResult,appliedJourney:applied});return {fakeResult,applied,presentation};}
...
async function runCase(name,message,{expectClarification=false,existingJourney=null,expectBothAgents=false,expectConflictHandling=false}={}){
 ...
 const intelligence=await interpret(message,journey);
 assert.ok(intelligence,"semantic intelligence missing");
...
async function run(){
 ...
 const relationship=(corrected.decisions||[]).find(d=>d.status==="active"&&d.authority==="creator"&&d.key==="movie.character.relationship");
 assert.ok(relationship,"creator correction did not produce active creator relationship");
 assert.equal(String(relationship.value).toLowerCase().includes("brother")||String(relationship.value).toLowerCase().includes("sister"),true,"creator correction did not supersede old relationship");
