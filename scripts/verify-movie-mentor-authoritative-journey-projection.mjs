...
const projected8 = engine.reconcileAuthoritativeCreatorTruth(journeyN, authority8);
...
const replay8 = engine.reconcileAuthoritativeCreatorTruth(projected8, structuredClone(authority8));
...
const replayAfterRestart = engine.reconcileAuthoritativeCreatorTruth(restarted, structuredClone(authority8));
assert.deepEqual(replayAfterRestart, restarted, "restart replay must remain idempotent");

assert.throws(() => engine.reconcileAuthoritativeCreatorTruth(projected8, { ...authority8, revision: 7 }), error => error.code === "CREATOR_JOURNEY_STALE_AUTHORITY");
...
const projected9 = engine.reconcileAuthoritativeCreatorTruth(restarted, authority9);
